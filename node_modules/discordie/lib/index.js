...
const RequestQueue = require("./core/ratelimiting/RequestQueue");
...
class Discordie {
  constructor(options) {
    ...
    this.token = null;
    this.bot = false; // transitioning to bot API, will be true in future
    ...
    this._messagePump = new RequestQueue();
    if (!this.options.messageQueue) {
      this._messagePump.disabled = true;
    }
     ...
     * (internally called guilds) current user has access to.
    ...
    /**
     * Interface to a collection containing all public channels current user
     * has access to.
    ...
    /**
     * Interface to a collection containing all users current user
     * has access to.
     *
     ...
     * channels current user has access to.
    ...
    this.UnavailableGuilds =
      new UnavailableGuildCollection(this, () => this.gatewaySocket);
